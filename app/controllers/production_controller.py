...
@jwt_required()
def production_by_id(production_id: int):
    session: Session = db.session
    Productions: Query = (
        session.query(Production)
            .select_from(Production)
            .filter(Production.production_id == production_id)
            .order_by(Production.production_id)
            .all())
    sezalized_production= []
    for production in Productions:
...
@jwt_required()
def production_updater(production_id: int):
    data = request.get_json()
    session: Session = db.session
    expected_keys={"recipe_id","recipe_quantity"}
    try:
...
@jwt_required()
def production_recipes_deleter(production_id: int):
    session: Session = db.session()
    productionsrecipe: ProductionRecipe = ProductionRecipe.query.filter_by(id = production_id).first()
...
@jwt_required()
def production_deleter(production_id: int):
    session: Session = db.session()
    productions: Production = Production.query.filter_by(production_id = production_id).first()
    if not productions:
        return {"Error": "id not found"}, HTTPStatus.NOT_FOUND
...
def gamma():
    ...
    return jsonify(lista_de_consumo), HTTPStatus.OK
